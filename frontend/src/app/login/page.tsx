...
export default function LoginPage() {
    ...
    // Steps: 'email' -> 'password' -> 'register_details'
    ...
    const handleSocialLogin = async (provider: string) => {
        try {
            ...
            if (provider === 'Google') {
                ...
                // To keep it simple for now, we'll alert for Phone as it requires Recaptcha UI
    ...
    const handleEmailSubmit = (e: React.FormEvent) => {
        ...
        setError('');
        if (!username.trim()) {
            setError('กรุณากรอกที่อยู่อีเมลหรือชื่อผู้ใช้');
            return;
        }
        setStep('password');
    };
    ...
    const handlePasswordSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err.message === '2FA_REQUIRED') {
                ...
                // Not found or incorrect password -> Suggest registration
                setError('ไม่พบบัญชีหรือรหัสผ่านไม่ถูกต้อง (หากคุณเป็นผู้ใช้ใหม่ กรุณาสร้างบัญชี)');
                // Instead of hard failing, we give them a button to register
    ...
    return (
        <main className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white font-sans antialiased relative transition-colors duration-300">
            {/* Background Ambient Glow (Optional matching other pages) */}
            ...
            {/* Modal Container */}
            <div className="relative z-10 w-full max-w-[440px] bg-white dark:bg-slate-800 rounded-[32px] p-8 md:p-10 shadow-xl border border-slate-100 dark:border-slate-700 mx-4 my-8 flex flex-col transition-colors duration-300">
                
                {/* Close Button */}
                <Link href="/" className="absolute top-6 right-6 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                    <span className="material-symbols-outlined text-[20px]">close</span>
                </Link>

                {/* Brand Logo */}
                <div className="flex justify-center mb-6 mt-2">
                    <div className="w-12 h-12 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shadow-sm border border-indigo-100 dark:border-indigo-800/50">
                ...
                {step === 'email' && (
                    <>
                        <div className="text-center mb-8">
                            <h1 className="text-2xl font-bold mb-2 tracking-tight">เข้าสู่ระบบ / สมัครบัญชี</h1>
                        ...
                        <div className="space-y-3 mb-6">
                            <button 
                                ...
                                className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl transition-colors text-[14px] font-semibold text-slate-700 dark:text-slate-200 shadow-sm"
                            >
                        ...
                        <form onSubmit={handleEmailSubmit} className="space-y-4">
                            ...
                            <button
                                type="submit"
                                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 rounded-xl font-bold text-[15px] shadow-md shadow-indigo-200 dark:shadow-none transition-all active:scale-[0.98]"
                            >
                                ดำเนินการต่อ
                            </button>
                        </form>
                ...
                {step === 'password' && (
                    <>
                        <div className="text-center mb-6">
                            <h1 className="text-2xl font-bold mb-3 tracking-tight">ป้อนรหัสผ่าน</h1>
                        ...
                        <form onSubmit={handlePasswordSubmit} className="space-y-5">
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 rounded-xl font-bold text-[15px] shadow-md shadow-indigo-200 dark:shadow-none transition-all active:scale-[0.98] disabled:opacity-50"
                            >
                ...
                {step === 'register_details' && (
                    <>
                        <div className="text-center mb-6">
                            <h1 className="text-2xl font-bold mb-3 tracking-tight">สร้างบัญชีใหม่</h1>
                        ...
                        <form onSubmit={handleRegisterSubmit} className="space-y-4">
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 rounded-xl font-bold text-[15px] shadow-md shadow-indigo-200 dark:shadow-none transition-all active:scale-[0.98] mt-2 disabled:opacity-50"
                            >
