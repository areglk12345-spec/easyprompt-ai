...
function ChatContent() {
    ...
    const startNewChat = () => {
        ...
        setMessages([
            { 
                role: 'agent', 
                text: 'สวัสดีครับ! ผมคือ V-Bot มาสคอตผู้ช่วยของคุณ วันนี้มีอะไรให้ผมช่วยร่างหรือสร้าง Prompt ไหมครับ? เช่น อยากให้ช่วยเขียนอีเมล, สรุปบทความ, หรือวางแผนเที่ยว บอกผมได้เลยครับ 🤖',
                ...
                if (response.ok) {
                    ...
                    if (data && data.length > 0) {
                        ...
                        setMessages([
                            { 
                                role: 'agent', 
                                text: 'สวัสดีครับ! ผมคือ Verbaqo Agent วันนี้มีอะไรให้ผมช่วยไหมครับ? เช่น อยากให้ช่วยเขียนอีเมล, สรุปบทความ, หรือวางแผนเที่ยว บอกมาได้เลยครับ',
                                suggestedOptions: ['เขียนจดหมาย/อีเมล', 'สรุปบทความ', 'แปลภาษา', 'วางแผนท่องเที่ยว']
                            }
                        ]);
                    }
    ...
    return (
        <div className={`fixed inset-0 overflow-hidden bg-transparent transition-all duration-300 ${isLarge ? 'text-base' : 'text-sm'}`}>
            <div className="flex h-full w-full overflow-hidden">
                ...
                <main 
                    className="flex-1 flex flex-col bg-white dark:bg-slate-900 overflow-hidden h-full relative transition-colors duration-300"
                    onDragOver={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                ...
                >
                    ...
                    <header className="flex justify-between items-center pl-16 pr-4 md:px-12 w-full h-auto min-h-[4rem] py-2 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-outline-variant/30 dark:border-slate-700/30 shrink-0 gap-2 flex-wrap z-30">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-bold text-slate-800 dark:text-white hidden md:inline">V-Bot · มาสคอตผู้ช่วยเขียน Prompt</span>
                        </div>
